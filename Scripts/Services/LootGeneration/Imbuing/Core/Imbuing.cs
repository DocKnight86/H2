    ...
    public class Imbuing
    {
        ...
        public static int GetTotalMods(Item item, int id = -1)
        {
            ...
            else if (item is BaseArmor armor)
            {
                ...
                if (armor.PhysicalBonus > 0 && id != 51) { total++; }
                if (armor.FireBonus > 0 && id != 52) { total++; }
                if (armor.ColdBonus > 0 && id != 53) { total++; }
                if (armor.PoisonBonus > 0 && id != 54) { total++; }
                if (armor.EnergyBonus > 0 && id != 55) { total++; }
            ...
            else if (item is BaseClothing clothing)
            {
                ...
                if (clothing.Resistances.Physical > 0 && id != 51) { total++; }
                if (clothing.Resistances.Fire > 0 && id != 52) { total++; }
                if (clothing.Resistances.Cold > 0 && id != 53) { total++; }
                if (clothing.Resistances.Poison > 0 && id != 54) { total++; }
                if (clothing.Resistances.Energy > 0 && id != 55) { total++; }
            }
        ...
        public static int GetTotalWeight(Item item, int id, bool trueWeight, bool imbuing)
        {
            ...
            {
                if (arm.PhysicalBonus > 0 && id != 51)
                {
                    weight += 100.0 / 15 * arm.PhysicalBonus;
                }
                if (arm.FireBonus > 0 && id != 52)
                {
                    weight += 100.0 / 15 * arm.FireBonus;
                }
                if (arm.ColdBonus > 0 && id != 53)
                {
                    weight += 100.0 / 15 * arm.ColdBonus;
                }
                if (arm.PoisonBonus > 0 && id != 54)
                {
                    weight += 100.0 / 15 * arm.PoisonBonus;
                }
                if (arm.EnergyBonus > 0 && id != 55)
                {
                    weight += 100.0 / 15 * arm.EnergyBonus;
                }
            }
